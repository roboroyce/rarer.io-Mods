...
document.addEventListener('keydown', (e) => {
    if (e.key.toLowerCase() !== 'm') { // Avoid conflict with map toggle
...
document.addEventListener('mousemove', (e) => {
    if (!K || !G) return;
    const canvasRect = fe.getBoundingClientRect();
...
setInterval(() => {
    ...
    let newMoveAngle = (dx !== 0 || dy !== 0) ? Math.atan2(dy, dx) : null;

    // Always send to ensure angle is updated
    f({ kind: PacketType.Input, angle: facingAngle, moveAngle: newMoveAngle });

    moveAngle = newMoveAngle;
}, 33); // Sync with server update rate